...
<div class="wrap">
    <h2>AWeber Integration Setting</h2>
...
<input type="hidden" id="awaber_list_name" name="awaber_list_name" value="" />
            <?php } else { ?>
...
jQuery(document).ready(function(){
	jQuery('#aweber-settings-save-button').click(function(){
		var peopleList=jQuery('#<?php echo $this->widgetOptionsName; ?>-list').find('option:selected').text();
		
		jQuery('#awaber_list_name').val(peopleList);
            ...
            <?php } else { ?>
                ...
                <p class="submit">
                    <input type="hidden" name="_wp_http_referer" value="<?php echo admin_url('admin.php?page=wpsp-autoresponder'); ?>" />
                    <input type="submit" id="aweber-settings-button" class="button-primary" value="<?php _e($button_value) ?>" />
                </p>
            <?php } ?>
