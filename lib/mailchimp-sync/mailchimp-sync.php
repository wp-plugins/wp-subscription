...
function mailchimp_settings_page_output() {
  ...
  if ( !current_user_can('edit_users') )
    wp_die('Nice try!');
    echo '<style> .collapse .updated {display:none;} </style>';
	...
	switch( $action ) {
		...
		default:
            ...
            if ( is_multisite() ) {
		          ...
		          ?><form method="post" action="settings.php?page=wpsp-autoresponder&action=process"><?php
		        } else {
		          ?><form method="post" action="ms-admin.php?page=wpsp-autoresponder&action=process"><?php
		        }
			} else {
  				?>
        			<form method="post" action="admin.php?page=wpsp-autoresponder&action=process">
       			<?php
			}
	        ...
	        if ( ! empty( $mailchimp_apikey ) ) {
				...
				if ( empty( $api->errorMessage ) ) {
					...
					update_option('mailchimp_api_setting','enabled');
				} else {
					?>
						<tr class="form-field form-required">
					...
					<?php
					update_option('mailchimp_api_setting','desabled');
		...
		case "process":
			...
			if ( is_multisite() ) {
				...
				echo "
					<SCRIPT LANGUAGE='JavaScript'>
					window.location='admin.php?page=wpsp-autoresponder&updated=true&updatedmsg=" . urlencode(__('Settings saved.', 'mailchimp')) . "';
					</script>
				";
			}
			break;
			...
			?><h2><?php _e('Syncing Existing Users', 'mailchimp') ?></h2>
      		...
      		<?php
				...
				if ( is_multisite() ) {
					...
					echo "
					<SCRIPT LANGUAGE='JavaScript'>
					window.location='admin.php?page=wpsp-autoresponder&updated=true&updatedmsg=" . urlencode($msg) . "';
					</script>
					";
				}
			break;
